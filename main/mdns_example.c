...
#define BUTTON_GPIO GPIO_NUM_0 // 0
...
static void initialise_mdns(void)
{
    ...
    mdns_txt_item_t serviceTxtData[3] = {
        {"device", "LNM_DSM4"},
        {"Mfg", "Jul-2020"},
        {"Mode", "HTTP:80-GET-POST,UDP:4567"}
    };

    printf("\n\nAbout to init mds ... \n");
    //initialize service
    ESP_ERROR_CHECK( mdns_service_add("DSM4_WebServer", "_http", "_tcp", 80, serviceTxtData, 3) );
    //add another TXT item
    ESP_ERROR_CHECK( mdns_service_txt_item_set("_http", "_tcp", "deviceID", hostname) );
    //change TXT item value
    ESP_ERROR_CHECK( mdns_service_txt_item_set("_http", "_tcp", "u:p", "Admin:Password") );
    free(hostname);
}
...
static char* generate_hostname(void)
{
    ...
    if (-1 == asprintf(&hostname, "%s_%02X%02X%02X%02X%02X%02X", MDNS_HOSTNAME, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5])) 
